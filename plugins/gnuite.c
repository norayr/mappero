...
#define GCONF_GNUITE_KEY_PREFIX GCONF_ROUTER_KEY_PREFIX"/gnuite"
#define GCONF_GNUITE_KEY_AVOID_HIGHWAYS GCONF_GNUITE_KEY_PREFIX"/avoid_highways"

#define GNUITE_ROUTER_URL \
    "http://www.gnuite.com/cgi-bin/gpx.cgi?saddr=%s&daddr=%s"

static void router_iface_init(MapRouterIface *iface);

G_DEFINE_TYPE_WITH_CODE(MapGnuite, map_gnuite, G_TYPE_OBJECT,
...
static const gchar *
map_gnuite_get_name(MapRouter *router)
{
    return "Gnuite";
}

static void
map_gnuite_run_options_dialog(MapRouter *router, GtkWindow *parent)
{
    MapGnuite *gnuite = MAP_GNUITE(router);
    GtkWidget *dialog;
    GtkWidget *btn_highways;

    dialog = map_dialog_new(_("Gnuite router options"), parent, TRUE);
...
static void
map_gnuite_calculate_route(MapRouter *router, const MapRouterQuery *query,
                           ...
                           gpointer user_data)
{
    ...
    map_path_init(&path);
    route_download_and_setup(&path, GNUITE_ROUTER_URL, from, to,
                             gnuite->avoid_highways, &error);
    if (!error)
    {
...
static void
map_gnuite_load_options(MapRouter *router, GConfClient *gconf_client)
{
    MapGnuite *gnuite = MAP_GNUITE(router);

    gnuite->avoid_highways = gconf_client_get_bool(gconf_client, GCONF_GNUITE_KEY_AVOID_HIGHWAYS, NULL);
}

static void
map_gnuite_save_options(MapRouter *router, GConfClient *gconf_client)
{
    MapGnuite *gnuite = MAP_GNUITE(router);

    gconf_client_set_bool(gconf_client, GCONF_GNUITE_KEY_AVOID_HIGHWAYS, gnuite->avoid_highways, NULL);
}

static void
router_iface_init(MapRouterIface *iface)
{
    iface->get_name = map_gnuite_get_name;
    iface->run_options_dialog = map_gnuite_run_options_dialog;
    iface->calculate_route = map_gnuite_calculate_route;
    iface->load_options = map_gnuite_load_options;
    iface->save_options = map_gnuite_save_options;
}

static void
map_gnuite_init(MapGnuite *gnuite)
{
}

static void
map_gnuite_class_init(MapGnuiteClass *klass)
{
}
...
map_plugin_get_object(const gchar *id)
{
    ...
    return g_object_new(MAP_TYPE_GNUITE, NULL);
}
